...
function Filter() {
  ...
  async function handlefilter(e) {
    ...
    try {
      const dietResponse = await axios.post("http://localhost:5000/user/diet-data", { age, height, weight });

      console.log("Diet API Response:", dietResponse.data); // ✅ Debugging
      ...
      console.error("Error fetching diet data:", error.response?.data || error.message);
    }
  }
  ...
  async function handleSave(e) {
    ...
    console.log(userId);
    const saveResponse = await axios.post("http://localhost:5000/user/diet", {
      userId,
      Breakfast,
      Lunch,
      Snacks,
        ...
        <div className="w-fit h-fit mb-6  flex flex-col justify-center items-center ">
          {data == null ? <div class="relative flex w-full animate-pulse gap-2 ">
            ...
            <div className="response-container sm:p-10 sm:m-10 p-5 m-5 md:p-20 md:m-20  rounded-lg bg-blue-50">
              <strong className='flex justify-center items-center text-lg mb-5'>{data.message}</strong>
              <p>
              ...
              <div className="flex justify-center items-center mt-10">
                <button className='w-fit flex justify-center items-center gap-[10px] bg-red-600 py-1.5 px-3 rounded-lg text-white' onClick={handleSave}> <p>Save</p> <IoSave /> </button>
              </div>
            </div>
          )}
        </div>
